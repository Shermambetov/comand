...
document.addEventListener("click", function (e) {
  if (e.target.classList.contains("btn-edit")) {
    ...
    fetch(`${API}/${id}`)
      .then((res) => res.json())
      .then((data) => {
        editImgInp.value = data.imgInp;
        editTitleInp.value = data.titleInp;
        editDescrInp.value = data.descrInp;
        editBtnSave.setAttribute("id", data.id);
      });
  }
});
...
editBtnSave.addEventListener("click", function () {
  let id = this.id;
  //   console.log(id);
  imgInp = editImgInp.value;
  titleInp = editTitleInp.value;
  descrInp = editDescrInp.value;

  if (!imgInp || !titleInp || !descrInp) {
    alert("Заполните поля");
    return;
  }
  let editedPost = {
